...
2. 完全关闭该工作簿的 WPS/Excel 编辑窗口，避免文件被锁定。
3. Mac 上双击 `一键同步mac.command`。首次如果拦截，请右键文件选择“打开”。

也可以用 Python 3.10+ 命令行运行：

```bash
python3 ./erp_excel_sync.py --config ./config.json
```

Mac 会自动打开一个独立的 ERP Chrome 窗口。首次或 ERP 登录过期时，只需在网页中正常登录；程序会自动检测登录成功并从该浏览器会话请求 ERP 数据，不需要复制或粘贴 Cookie。专用 Chrome 使用“恢复上次会话”保留登录状态：运行时自动打开，任务成功后自动关闭，下次运行无需重新登录；发生错误时保留窗口便于检查。
